...
long currentPanSteps;
long currentTiltSteps;
long currentMMSteps;
...
void panDegreesRel(float angle) {
  currentPanSteps = stepper_pan.currentPosition();
  target_position[0] = currentPanSteps + panDegreesToSteps(angle);
...
void tiltDegreesRel(float angle) {
  currentTiltSteps = stepper_tilt.currentPosition();
  target_position[1] = currentTiltSteps + tiltDegreesToSteps(angle);
...
void sliderMoveToRel(float mm) {
  currentMMSteps = stepper_slider.currentPosition();
  target_position[2] = currentMMSteps - sliderMillimetresToSteps(mm);
...
void positionReport(void) {
  printi(F("Position Report   : "));
  printi(F("Pan angle         : "), panStepsToDegrees(stepper_pan.currentPosition()), 3, F("°"));
  delay(200);
  printi(F("Tilt angle        : "), tiltStepsToDegrees(stepper_tilt.currentPosition()), 3, F("°"));
  printi(F("Slider position   : "), sliderStepsToMillimetres(stepper_slider.currentPosition()), 3, F(" mm\n"));
  delay(200);
}
...
void serialData(void) {
  ...
  switch (instruction) {
    ...
    case INSTRUCTION_SLIDER_MILLIMETRES_REL: {
        sliderMoveToRel(serialCommandValueFloat);
      }
      break;
